...
{
	float m = 1.0+(u_mouse.x/u_resolution.x)*300.0;
	t = (u_time*1.5+m+657.0)*.006 + t;
...
{
    vec2 xy = -1.0 + 2.0*fragCoord.xy / u_resolution.xy;
	vec2 uv = xy * vec2(u_resolution.x/u_resolution.y,1.0);
	vec3 camTar;
	...
	cameraPos.xz = vec2(0, 0);
//	camTar.xyz	 = CameraPath(.1).xyz;
//	camTar.y = cameraPos.y = max((h*.25)+3.5, 1.5+sin(u_time*5.)*.5);
	//camTar.y = cameraPos.y = (h*.25)+3.5;
//    camTar.y -= smoothstep(60.0, 300.0,cameraPos.y)*150.;
	cameraPos.y = 100;
	camTar = vec3(-100, -100, -100);

	float roll = 0.15*sin(u_time*.2);
